...
class Constants {
    struct MapViewConstants {
        ...
        static let MaxNumberOfPhotosToSavePerPin = 36  //i decided to use 36 because it seems like a reasonable size for an image collection, and also divisible by 3 and 4 (which are the number of columsn of photos within the collection view when in portrait and landscape modes); one enhancement i would make if i were to fully develop this app would be to enable the user to decide this value
    }
